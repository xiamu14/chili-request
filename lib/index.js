        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    axios.interceptors.request.use(function (configInstance) {
                        // NOTE: 为每一次请求设置 cancelToken
                        var cancelTokenSource = axios.CancelToken.source();
                        // @ts-ignore
                        configInstance.cancelToken = cancelTokenSource.token;
                        var option = regConfig.option;
                        if (option.cache && option.cache.type === 'session') {
                            // 尝试获取缓存数据
                            var data_1 = sessionStorage.getItem(option.url + "-" + JSON.stringify(option.data) + "-" + option.method);
                            // console.log('不可思议啊', data);
                            // NOTE: 命中缓存
                            if (data_1) {
                                cancelTokenSource.cancel(data_1);
                            }
                        }
                        // @ts-ignore
                    ...
                    axios.interceptors.response.use(function (res) {
                        try {
                            ...
                            // @ts-ignore
                            if (option.cache && option.cache.type === 'session') {
                                sessionStorage.setItem(option.url + "-" + JSON.stringify(option.data) + "-" + option.method, JSON.stringify(res.data));
                            }
                            // @ts-ignore
...
function chiliReqBase$1(config) {
    var _this = this;
    axios.defaults.baseURL = config.baseURL;
    // NOTE:发送前拦截，用于获取 token 等...
...
var ChiliReqContext = React__default.createContext(undefined);
function useChiliReq() {
    var config = React.useContext(ChiliReqContext);
    if (config === undefined) {
        throw 'chiliReq missing baseConfig.';
    }
    return function () { return chiliReqBase$1(config); };
}
function useFetch() {
    var config = React.useContext(ChiliReqContext);
    if (config === undefined) {
    ...
    var ref = React.useRef(chiliReqBase$1(config));
    return ref;
}

exports.ChiliReqContext = ChiliReqContext;
exports.default = chiliReqBase;
exports.useChiliReq = useChiliReq;
